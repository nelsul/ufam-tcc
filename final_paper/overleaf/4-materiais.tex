...
RNF002 & Usabilidade & A interface da tela de solicitaçao deve ser responsiva e adaptável a dispositivos móveis, tablets e desktops. \\ \hline
RNF003 & Segurança & As senhas dos usuários devem ser armazenadas utilizando hash. \\ \hline
RNF004 & Segurança & Os prontuários eletrônicos devem ser armazenados de forma criptografada no banco de dados. \\ \hline
RNF005 & Segurança & O acesso aos dados de alunos deve ser restrito apenas aos profissionais autorizados. \\ \hline
RNF006 & Segurança & Professores devem visualizar apenas observações de estudantes matriculados em suas disciplinas. \\ \hline
RNF007 & Internacionalização & O sistema deve oferecer suporte a múltiplos idiomas (Português, Inglês, Espanhol). \\ \hline
RNF008 & Confiabilidade & O sistema de envio de emails deve utilizar fila assíncrona para garantir entrega. \\ \hline
RNF009 & Confiabilidade & O sistema de emails deve implementar retry automático em caso de falhas temporárias. \\ \hline
RNF010 & Usabilidade & Todas as notificações por email devem ser enviadas em português brasileiro. \\ \hline
RNF011 & Usabilidade & As datas e horários devem ser exibidas no fuso horário de Manaus (UTC-4). \\ \hline
RNF012 & Desempenho & O sistema deve utilizar índices de banco de dados para otimizar atendimentos frequentes. \\ \hline
RNF013 & Manutenibilidade & O código deve seguir padrões de nomenclatura e boas práticas de desenvolvimento. \\ \hline
RNF014 & Manutenibilidade & Tipos de sessão e observações devem ser gerenciáveis sem alteração de código. \\ \hline
RNF015 & Acessibilidade & A interface deve seguir princípios básicos de acessibilidade (contraste, navegação por teclado). \\ \hline
RNF016 & Portabilidade & O sistema deve ser containerizável usando Docker para facilitar implantação. \\ \hline
\end{longtable}
}
...
\subsection{Esquema do Banco de Dados}
...
\begin{enumerate}
    ...
    \item \textbf{session\_types (Tipos de Sessão)}: Tabela de configuração que define os tipos de atendimento disponíveis (ex: "Primeiro Atendimento", "Retorno", "Orientação"). Cada tipo possui nome, descrição e duração padrão em minutos.
    ...
    \item \textbf{patient\_observations (Observações de Alunos)}: Registra observações específicas feitas por profissionais sobre estudantes. Estas observações podem ser compartilhadas de forma controlada com professores que lecionam para o estudante em questão.
...
\textbf{Índices Otimizados}: O banco possui índices estratégicos em campos frequentemente utilizados em atendimentos:

\begin{itemize}
    \item Índices em chaves estrangeiras para otimizar joins
    \item Índices compostos em \texttt{(start\_time, end\_time)} para atendimentos de disponibilidade e agendamentos
